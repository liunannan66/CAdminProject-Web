...
import { resolveRouteIcon } from '@/router/routes/normalize-backend-routes';
...
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | ElegantConstRoute) {
  ...
  const { title, i18nKey, localIcon, iconFontSize } = route.meta ?? {};
  const icon = resolveRouteIcon(route as import('@elegant-router/types').ElegantConstRoute);
