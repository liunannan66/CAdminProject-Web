...
<template>
  <ElSpace direction="vertical" fill :size="16">
    ...
    <ElCard header="权限钩子" class="card-wrapper">
      <ElSpace>
        <p>具有`api:SysConfig:Add`权限可见，使用v-if="hasAuth('api:SysConfig:Add')"</p>
        <ElButton v-if="hasAuth('api:SysConfig:Add')">添加</ElButton>
      </ElSpace>
    </ElCard>
  </ElSpace>
</template>
